...
pub(crate) async fn audit(
    ...
    // Build a `RegistryClient` for the project-status audit; mirrors the
    // pattern used by `tree` and other commands that need cached Simple-API
    // access outside of the resolver.
    ...
    // Run the vulnerability query and the project-status query concurrently.
    // Status failures are swallowed inside `ProjectStatusAudit`, so only OSV
    // can produce a hard error here.
