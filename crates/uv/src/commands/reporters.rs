...
impl PublishReporter {
    ...
    pub(crate) fn new(printer: Printer, length: Option<u64>) -> Self {
        let target = progress_target(printer);
        let multi_progress = MultiProgress::with_draw_target(target.clone());
        let root = multi_progress.add(ProgressBar::with_draw_target(length, target));
        let embedded_root = embedded_progress::start_task(
            None,
