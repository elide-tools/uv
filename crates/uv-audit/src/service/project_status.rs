...
impl<'a> ProjectStatusAudit<'a> {
    ...
    /// Query the project-level status of each `(name, index)` pair, returning
    /// one [`Finding::ProjectStatus`] per project whose index-reported status
    /// is not [`Status::Active`].
    ///
    /// Transient per-project query failures (network errors, not-found, offline
    /// without a cache hit) are logged and dropped, on the rationale that one
    /// misbehaving index should not invalidate the rest of the audit.
    /// Flat-index responses carry no project-status information and are also
    /// silently skipped.
...
/// Map a PEP 792 [`Status`] to its [`AdverseStatus`] counterpart, or `None`
/// for [`Status::Active`].
